...
struct AppSettings {
    ...
    bool  inv_auto;
    bool  stop_auto;
    int   decoder_path;  // B246: 0=A narrow, 1=B wide, 2=HYB avg
    bool  nn_enable;     // B261
    bool  notch_enable;  // B262
    bool  vit_enable;    // B262
};
...
extern volatile uint32_t shared_c1_dma_wait_time;  // accumulated DMA wait μs, subtracted from Core 1 load
...
// Phase 9 Stage 3: dual IQ path selector (0=A narrow, 1=B wide, 2=HYB weighted)
extern volatile int   shared_decoder_path;
// Stage 3.2: HYB fusion weights — power-domain weighted sum.
// wa + wb should sum to 1.0 for AGC stability (enforced by WEIGHTS command).
extern volatile float shared_fuse_wa;
extern volatile float shared_fuse_wb;
// Stage 3.3: dynamic LLR fusion. When true, HYB uses per-path SNR estimates as weights.
extern volatile bool  shared_dyn_fusion;
extern volatile bool  shared_spectral_nr;
extern volatile float shared_snr_a_ema;
extern volatile float shared_snr_b_ema;
extern volatile float shared_agc_gain;
extern volatile bool  shared_agc_enabled;
extern volatile bool  shared_serial_diag;
extern volatile bool  shared_dump_frames;
...
// B261: TinyML neural net character classifier
extern volatile bool  shared_nn_enable;   // true = use NN decision instead of hard threshold

// B262: pre-DSP filter toggles (exposed via FILT pop-up + serial NOTCH/VIT cmds)
extern volatile bool  shared_notch_enable;  // LMS-notch chain (B244) bypass when false
extern volatile bool  shared_vit_enable;    // Soft-Viterbi frame validation (B243) bypass when false
