...
void core1_main() {
    ...
    int  path_ui_state = 0;  // 0=A 1=B 2=HYB 3=HYB+NN — UI shadow of (decoder_path, nn_enable)
    ...
    // path_ui derives from shared_decoder_path + shared_nn_enable (loaded above
    // by serial_commands settings handler). NN flag is currently ON-by-default.
    path_ui_state = shared_nn_enable ? 3 : (shared_decoder_path & 0x3);
    ...
    ui.drawBottomBar(shared_baud_idx, shared_shift_idx, get_stop(), shared_afc_on, menu_mode, shared_search_state, shared_stop_auto, shared_baud_auto, shared_inv_auto, shared_rtty_inv);
    ...
    while (true) {
        ...
        // Re-sync path_ui_state from shared vars so serial NN ON/OFF and PATH X
        // commands are reflected on display (top bar + menu) without a touch event.
        {
            int derived = shared_nn_enable ? 3 : (shared_decoder_path & 0x3);
            if (derived != path_ui_state) {
                path_ui_state = derived;
                if (menu_mode) ui.drawMenu(display_mode, "DIAG", path_ui_state, shared_notch_enable, shared_vit_enable);
            }
        }
        ...
        if (shared_save_request) {
            ...
            s.decoder_path = shared_decoder_path;
            s.nn_enable = shared_nn_enable; s.notch_enable = shared_notch_enable; s.vit_enable = shared_vit_enable;
        ...
        bool ui_active = !diag_screen_active && !menu_mode && !tuning_lab_active
                         && !reset_confirm_mode && !shift_popup_active
                         && !stop_popup_active && !baud_popup_active;

        if (rtty_char_ready) {
            char c = rtty_new_char;
            ui.addRTTYChar(c, /*is_error=*/false, ui_active);
        ...
        if (shared_err_flag) {
            ...
            // B263: compact red '*' on display, [ERR] still goes to serial
            // (cer_analyze + bench tooling parse it via re.sub).
            ui.addRTTYChar('*', /*is_error=*/true, ui_active);
        ...
        if (new_data_ready) {
            ...
            for (int i = 0; i < FFT_SIZE / 2; i++) shared_fft_mag[i] = smooth_mag[i];
            ...
            if (shared_search_request) {
                ...
                for (int j = 0; j < scan_count; j++) {
                    ...
                    // Wider shifts suffer more FSK spectral smearing and TX
                    // frequency drift — allow bigger tolerance so real signal
                    // at slightly-off bin count still matches.
                    int local_tolerance = tolerance;
                    if (shift_bins >= 40) local_tolerance = 4;      // 400+ Hz
                    else if (shift_bins >= 15) local_tolerance = 3; // 150+ Hz
                    for (int lo = 5; lo < FFT_SIZE/2 - shift_bins - local_tolerance; lo++) {
                        ...
                        for (int d = -local_tolerance; d <= local_tolerance; d++) {
                            ...
                            // Valley test: real FSK has spectral smearing between
                            // Mark/Space (peaks ~20-30 dB above noise). Two *different*
                            // signals whose frequency spacing happens to match a
                            // standard shift produce a deep valley of pure noise
                            // between them. Reject those false pairs.
                            // Only applied for shifts wide enough to measure (>20 bins),
                            // so narrow shifts (85/170 Hz) aren't affected.
                            if (shift_bins > 20) {
                                float peaks_avg = (lo_mag + hi_mag) * 0.5f;
                                float valley_min = 1e9f;
                                for (int k = lo + 2; k <= hi - 2; k++) {
                                    if (smooth_mag[k] < valley_min) valley_min = smooth_mag[k];
                                }
                                // If the valley drops >25 dB below the peaks average
                                // relative to the signal's own peaks, this is two
                                // separate signals, not one FSK pair.
                                if (valley_min < peaks_avg - 40.0f) continue;
                            }
            ...
            if (tuning_lab_active) {
                // Full-width dual-window eye diagram (2 bit periods side-by-side)
                spectrum.fillSprite(PAL_BG);
                ui.drawEyeDiagram(spectrum, 480, UI_DSP_ZONE_H);
        ...
        if (loop_start - last_touch > 50000) {
            ...
            if (is_touched && time_us_32() > touch_ignore_until) {
                if (ty >= UI_Y_MARKER && ty <= (UI_Y_DSP + UI_DSP_ZONE_H)) {
                    ...
                    else if (btn_idx == 3) {  // NOR/INV cycle: NOR → INV → NOR[A] → NOR (was idx=4 pre-B262)
                        flag_settings_change();
                        if (!shared_inv_auto && !shared_rtty_inv) {
                            shared_rtty_inv = true; shared_inv_uncertain = false;
                            printf(">> INV INV (manual)\n");
                        } else if (!shared_inv_auto && shared_rtty_inv) {
                            shared_inv_auto = true; shared_inv_uncertain = false;
                            printf(">> INV AUTO\n");
                        } else {
                            shared_inv_auto = false; shared_rtty_inv = false; shared_inv_uncertain = false;
                            printf(">> INV NOR (manual)\n");
                        }
                    }
                    else if (btn_idx == 4) { flag_settings_change(); shared_afc_on = !shared_afc_on; }  // AFC (was idx=3 pre-B262)
                    else if (btn_idx == 5) { shared_rtty_inv = false; shared_afc_on = true; shared_search_request = true; shared_search_state = 1; }
                    else if (btn_idx == 6) { ui.clearRTTY(); shared_clear_dsp = true; }
                    else if (btn_idx >= 7) {
                        if (tuning_lab_active) {
                    ...
                    if (baud_popup_active && !was_touched) {
                        ...
                        else if (btn == 1) { diag_screen_active = true; menu_mode = false; }
                        else if (btn == 2) { tuning_lab_active = true; menu_mode = false; }
                        else if (btn == 3) {  // PATH: A → B → HYB → HYB+NN → A
                            flag_settings_change();
                            path_ui_state = (path_ui_state + 1) & 0x3;
                            shared_decoder_path = (path_ui_state == 3) ? 2 : path_ui_state;
                            shared_nn_enable    = (path_ui_state == 3);
                            const char* lbl = path_ui_state == 0 ? "A"
                                            : path_ui_state == 1 ? "B"
                                            : path_ui_state == 2 ? "HYB"
                                            :                      "HYB+NN";
                            printf(">> PATH=%s\n", lbl);
                        }
                        else if (btn == 4) {  // NOTCH toggle (inline, no popup)
                            shared_notch_enable = !shared_notch_enable; flag_settings_change();
                            printf(">> NOTCH %s\n", shared_notch_enable ? "ON" : "OFF");
                        }
                        else if (btn == 5) {  // VIT toggle (inline, no popup)
                            shared_vit_enable = !shared_vit_enable; flag_settings_change();
                            printf(">> VIT %s\n", shared_vit_enable ? "ON" : "OFF");
                        }

                        if (menu_mode && !reset_confirm_mode) ui.drawMenu(display_mode, "DIAG", path_ui_state, shared_notch_enable, shared_vit_enable);
                        touch_ignore_until = time_us_32() + 300000;
        ...
        if (loop_start - c1_last_measure >= 500000) {
            // DMA waits block Core 1 but aren't real compute — subtract them
            // to match neighbor reference's measurement (honest load metric).
            uint32_t dma_wait = shared_c1_dma_wait_time;
            if (dma_wait > c1_total_work) dma_wait = c1_total_work;
            shared_core1_load = ((c1_total_work - dma_wait) * 100.0f) / (float)(loop_start - c1_last_measure);
            c1_total_work = 0; c1_last_measure = loop_start;
            shared_c1_dma_wait_time = 0;
        }
