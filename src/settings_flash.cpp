...
void settings_load() {
    ...
    if (loaded_set.decoder_path >= 0 && loaded_set.decoder_path <= 2)
        shared_decoder_path = loaded_set.decoder_path;
    shared_nn_enable    = loaded_set.nn_enable;
    shared_notch_enable = loaded_set.notch_enable;
    shared_vit_enable   = loaded_set.vit_enable;
}
...
void settings_build_from_state(AppSettings& s, int display_mode, bool auto_scale) {
    ...
    s.nn_enable    = shared_nn_enable;
    s.notch_enable = shared_notch_enable;
    s.vit_enable   = shared_vit_enable;
}
