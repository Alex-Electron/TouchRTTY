...
С прошивкой едет маленький (7→128→64→32, около 44 КБ float32-весов)
...
фреймы, которые inference-time gate пустит в NN. Это фокусирует
...
Чтобы сдвинуть стрелку через real data, надо размечать
...
Каждый из них прошёл multi-seed bench. Артефакты лежат на стенде, в
репо не закоммичены (раздуют его на полгигабайта), но рецепт каждого
варианта описан выше и воспроизводим — `tools/train_nn_torch.py` плюс
флаги из колонки «Что пробовали» дают те же веса.
