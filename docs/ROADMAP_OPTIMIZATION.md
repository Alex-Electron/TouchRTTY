...
### Авто-инверсия Mark/Space — DONE (Build 196)
- [x] Фоновый детектор: если ERR > 40% при открытом SQ в течение 3 сек → пробуем инвертировать
- [x] Замер 3 сек после инверсии: если ERR не снизился < 35% → откат
- [x] Работает полностью автоматически, без вмешательства пользователя
- [x] Сброс счётчика ошибок при каждой попытке для чистого замера

### Автопоиск сигнала (по кнопке SEARCH) — TODO
Запускается кнопкой, не работает постоянно. AFC остаётся на постоянной основе.
- [ ] Кнопка **SEARCH** в меню или Tuning Lab
- [ ] Сканирование FFT: поиск пары пиков с расстоянием = стандартный шифт (170/200/425/450/850 Гц)
- [ ] Критерий: оба пика SNR > порога, расстояние соответствует одному из шифтов
- [ ] При нахождении: установка центральной частоты и шифта
- [ ] Отображение прогресса: "SEARCHING..." с анимацией

### Авто-определение шифта (по FFT пикам) — TODO
- [ ] Два самых ярких пика в FFT → расстояние = шифт
- [ ] Сравнение с таблицей стандартных шифтов, выбор ближайшего
- [ ] Может работать совместно с автопоиском или отдельной кнопкой
...
### Авто-определение стоп-бита — TODO
- [ ] Статистический анализ: при 1.0 стоп-бите и настройке 1.5 — систематический сдвиг фазы DPLL
- [ ] Перебор 1.0/1.5/2.0, замер ERR, выбор минимального
- [ ] Низкий приоритет — разница между 1.0 и 1.5 мала (~10мс при 50 Бод)

### Полный автодетект (кнопка AUTODETECT) — TODO
Объединяет всё: поиск сигнала → определение шифта → скорости → инверсии → стоп-бита.
- [ ] Последовательный pipeline: SEARCH → SHIFT → BAUD → INV → STOP
- [ ] Прогресс на экране по шагам
- [ ] Итог: "Found: 50 Baud, 450 Hz shift, 1.5 stop, LSB" → применить и сохранить
