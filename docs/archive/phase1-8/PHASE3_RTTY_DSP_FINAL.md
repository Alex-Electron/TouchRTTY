...
> **Status: Архив (v1.x snapshot, Build 218).** Это документация
> single-IQ декодера, который шёл в **v1.72** и младше. В **v2.0.0
> (Build B265)** этот pipeline переработан в Phase 9 — dual-IQ + LLR
> fusion + Soft-Viterbi + TinyML NN. Актуальная архитектура описана
> в `RELEASE_v2.0.0.md` и `docs/PHASE9_HYBRID_DECODER_PLAN.md`. Этот
> файл сохранён как опорный материал по single-IQ части (она же
> базовая ветка path A в Phase 9).
