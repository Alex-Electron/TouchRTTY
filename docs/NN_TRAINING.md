...
We ship a small (7→128→64→32, about 44 KB of float32 weights) MLP
...
It's reasonable to ask why we ship an NN for what's essentially a 5-bit
...
soft-bit values not always cleanly bipolar — and at the SNRs we care
...
inference we only invoke the network when the weakest data bit's
magnitude is below 30 % of the estimated signal level. Above that we
...
Where `nn_margin = top_logit − second_top_logit`. We trust the NN's
...
that mattered most. We landed on it after `noise_mean=0.28` produced
...
the actual reason we ported to PyTorch.
...
matter. Wide σ would mean we got lucky on one seed; small σ means
the win is robust.
...
We learned this the hard way. Validation accuracy on the synthetic
held-out set sits around 89–91 % for almost every variant we tried.
...
| `--label-smoothing <float>` | Cross-entropy label smoothing. We tried 0.05; it hurt at −14 and −16. |
| `--weight-uncertain <float>` | Per-sample weight multiplier for `data_min < 0.30` frames. **3.0 won; 5.0 overshot.** |
| `--dropout <float>` | Dropout. We tried 0.1; marginal, increased variance. |
...
* `TRAIN_NN_H1`, `TRAIN_NN_H2` — change hidden layer widths (default
  ...
  so we didn't adopt it. If you've got specific noise patterns the
...
⚠️ **A caveat we learned the hard way.** Our first real-air-augmented
...
help, because we wanted the NN to *outperform* hard decision on hard
frames.
...
We used it to run six variants overnight unattended.
...
us can re-check the experiments without having to repeat them:

| Variant | What we tried | Why it failed |
|---|---|---|
...
first — there's a good chance someone tried something similar and we
