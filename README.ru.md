...
Карманный RTTY-декодер, которому можно доверять на слабом сигнале.
...
> [!IMPORTANT]
> Нужен именно **Pico 2 (RP2350)** — не старый RP2040. Используется в проекте 
> FPU от Cortex-M33 и больше SRAM, чем влезает в старый чип.
...
Если ничего больше не трогаешь и хочешь сразу отличный декодер —
...
* **Нейронка наконец помогает.** При моих первых попытках NN была неоднозначной —
...
* **UI стал проще.** NOTCH и VIT — теперь тоглы прямо в главном меню,
  а не закопаны в подвал. Ошибки frame-rejection показываются на экране
...
## Как результаты сравнения с 2Tone
...
| Просто запусти тестовый сигнал в браузере | [`tools/rtty_simulator.html`](tools/rtty_simulator.html) |
...
* **DRM** — Digital Radio Mondiale, на более долгий горизонт (понавдобится аппаратная модификация).
* **IQ-direct вход** — сможем подавать квадратурное IQ прямо с SDR-приемника (например, Belka-DX) или WebSDR (конечно, только те, кто умеет IQ отдавать) в ADC Pico, минуя аудио-тракт и AGC-клиппинг. +2–4 dB в маргинальных условиях и обязательное условие для DRM/wideband работ выше.
* **UI палитры / скины** — косметика, цветовые палитры, например, «Hacker green» и может что-то еще для улучшения визуала.

---

## Как обрабатывается

Подаем наш аудиосигнал, обязательно ground-referenced (1.65 В bias, line level) в GP26.
Дальше так:
...
Core 0 занят 10 кГц hard-real-time циклом (около 7 % CPU). Core 1
отвечает за UI, 1024-point FFT для водопада, тач и USB-консоль (около
...
Если `picotool` не настроен или скомпилировать не выходит - кидай получившийся `.uf2` на
mass-storage диск `RPI-RP2` по-старинке: зажми BOOTSEL и подключи Pico.

---

## Quick start: сигнал → текст

1. Подключи дисплей, тач и bias-цепь на аудио - пины в
   [Hardware setup](docs/HARDWARE_SETUP.ru.md).
2. Прошей `TouchRTTY_v2.0.0.uf2`.
3. Подай аудио — линейный выход PC, AF-jack настоящей рации или
   WebSDR в браузере через виртуальный аудиокабель.
4. Нажми на тачскрине **SEARCH**. Скан 300–3000 Гц, ловит самый подобный на RTTY пик.
...
Текст пойдёт в среднюю зону экрана. Serial-консоль зеркалит его и вставляет `[ERR]` там, где декодер отверг фрейм.
...
DSP-код — из тех, что заметно улучшается, когда люди его гоняют
...
## Коммерческое использование (Commercial licensing)

Стандартная лицензия для TouchRTTY — GPL v3. Вы можете свободно использовать,
модифицировать и распространять код при условии, что ваш производный продукт
также выпускается под лицензией GPL v3 с указанием авторства этого проекта.

Если вы хотите использовать TouchRTTY (или его существенные части) в
коммерческом продукте — например, продавать собранные устройства или
поставлять его в составе закрытой прошивки — пожалуйста, свяжитесь
со мной по адресу lavrinovich.alex@gmail.com для обсуждения отдельной коммерческой лицензии. Такие
лицензии обсуждаются индивидуально и обычно предполагают
роялти или единоразовый платёж.
